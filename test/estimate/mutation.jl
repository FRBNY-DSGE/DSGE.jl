...
s_part1, eps_part1, acpt = mutation(m,[50.2,8.3,7.6],[.8,.9,.6,.9,.11,5,7,10],[.2,.5,.7],A,B,R,Φ,H,sqrtS2,cov_mat)
...
s, eps, acpt = mutation(m, data[:,1], ones(8), zeros(3),A,B,R,Φ,H,sqrtS2,cov_mat)  
...
ind_s, ind_eps, ind_acpt = mutation(m, data[:,1], [-0.6119,-1.5262,-2.6471,0.9952,-0.8350,-0.4785,-0.3428,-1.6893], zeros(3),A,B,R,Φ,H,sqrtS2,cov_mat)
@test ind_acpt == 1

#Test unreasonable case (should reject)
ind_s, ind_eps, ind_acpt = mutation(m, data[:,1], 3*[-0.6119,-1.5262,-2.6471,0.9952,-0.8350,-0.4785,-0.3428,-1.6893], zeros(3),A,B,R,Φ,H,sqrtS2,cov_mat)
@test ind_acpt == 0
