...
@testset "Testing getting MeansBands Input and Output Files" begin
    @test get_meansbands_input_file(m, :mode, :none, :histobs) == joinpath(rawpath(m, "forecast"),"histobs_cond=none_para=mode_vint="*data_vintage(m)*".jld2")
    @test get_meansbands_input_file("a", ["b"], :mode, :none, :histobs, fileformat = :jld2) == joinpath("a","histobs_b_cond=none_para=mode.jld2")

    @test get_meansbands_output_file(m, :mode, :none, :histobs) == joinpath(workpath(m, "forecast"),"mbhistobs_cond=none_para=mode_vint="*data_vintage(m)*".jld2")
    @test get_meansbands_output_file("a", ["b"], :mode, :none, :histobs, fileformat = "jld2") == joinpath("a","mbhistobs_b_cond=none_para=mode.jld2")
