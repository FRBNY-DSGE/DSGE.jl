...
@testset "Smoothing with regime switching" begin
    ...
    for smoother in [:hamilton, :koopman, :carter_kohn, :durbin_koopman]
        ...
        if smoother == :durbin_koopman
            ...
            @test @test_matrix_approx_eq pseudo_rs3[smoother] exp_pseudo_regime_switch
        else
            @test maximum(abs.(states_rs3[smoother] - exp_states_regime_switch)) < 4e-1
            @test maximum(abs.(shocks_rs3[smoother] - exp_shocks_regime_switch)) < 1e-1
            @test maximum(abs.(pseudo_rs3[smoother] - exp_pseudo_regime_switch)) < 3e-1
        end
    end
