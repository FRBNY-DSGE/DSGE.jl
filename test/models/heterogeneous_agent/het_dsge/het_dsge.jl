...
if check_jacobian
    ...
    @testset "Check indices" begin
        ...
        @testset "scalar-valued jumps" begin
            ...
            @test LELL  == endo[:LELL]
            @test LM    == endo[:LM]
            @test KK    == first(endo[:KK])
            @test LRR   == first(endo[:LRR])
            @test LII   == first(endo[:LII])
            @test LPI   == first(endo[:LPI])
            @test L2PI  == first(endo[:L2PI])
            @test L3PI  == first(endo[:L3PI])
            @test LY    == first(endo[:LY])
            @test L2Y   == first(endo[:L2Y])
            @test L3Y   == first(endo[:L3Y])
            @test L4Y   == first(endo[:L4Y])
            @test LZ    == first(endo[:LZ])
            @test L2Z   == first(endo[:L2Z])
            @test L3Z   == first(endo[:L3Z])
            @test LW    == first(endo[:LW])
            @test LX    == first(endo[:LX])
            @test B     == first(endo[:B])
            @test G     == first(endo[:G])
            @test Z     == first(endo[:Z])
            ...
            @test ELL   == endo[:ELL]
            @test M     == endo[:M]
            @test RR    == first(endo[:RR])
            @test II    == first(endo[:II])
            @test TT == first(endo[:TT])
            @test W == first(endo[:W])
            @test HH == first(endo[:HH])
            @test PI == first(endo[:PI])
            @test PIW == first(endo[:PIW])
            @test LAM == first(endo[:LAM])
            @test Y == first(endo[:Y])
            @test X == first(endo[:X])
            @test MC == first(endo[:MC])
            @test Q == first(endo[:Q])
            @test RK == first(endo[:RK])
        end
