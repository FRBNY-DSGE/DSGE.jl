...
@testset "Constructing a DSGE-VECM" begin
    ...
    @test subspec(m) == "ss1"
    @test !isempty(m.observables)
    @test isempty(m.cointegrated) # but cointegrated variables still empty
    @test !isempty(m.shocks)
...
@testset "Helper functions for DSGE-VECM" begin
    ...
    @test collect(keys(m.shocks)) == [:rm_sh] && isempty(m.observables) && m.lags == 0 && m.λ == 0. && isempty(m.cointegrated)
    DSGE.update!(m; observables = [:obs_gdp])
    @test collect(keys(m.shocks)) == [:rm_sh] && collect(keys(m.observables)) == [:obs_gdp] && m.lags == 0 && m.λ == 0. && isempty(m.cointegrated)
    DSGE.update!(m; lags = 1)
    @test collect(keys(m.shocks)) == [:rm_sh] && collect(keys(m.observables)) == [:obs_gdp] && m.lags == 1 && m.λ == 0. && isempty(m.cointegrated)
    DSGE.update!(m; λ = 0.5)
    @test collect(keys(m.shocks)) == [:rm_sh] && collect(keys(m.observables)) == [:obs_gdp] && m.lags == 1 && m.λ == 0.5 && isempty(m.cointegrated)
    DSGE.update!(m; cointegrated = [:obs_gdp])
    @test collect(keys(m.shocks)) == [:rm_sh] && collect(keys(m.observables)) == [:obs_gdp] && m.lags == 1 && m.λ == 0.5 && collect(keys(m.cointegrated)) == [:obs_gdp]
    ...
    @test_throws ErrorException DSGE.update!(m, dsge; cointegrated = [:not_an_observable])
    @test_throws AssertionError DSGE.update!(m, dsge; lags = -1)

    # Check access and size functions
    m = DSGE.DSGEVECM(dsge, collect(keys(dsge.exogenous_shocks)), "ss0")
    DSGE.update!(m; observables = [:obs_spread], lags = 1, cointegrated = [:obs_spread])
    @test collect(keys(DSGE.get_observables(m))) == [:obs_spread]
    @test collect(keys(DSGE.get_cointegrated(m))) == [:obs_spread]
    @test n_observables(m) == 1
    @test DSGE.n_cointegrated(m) == 1
