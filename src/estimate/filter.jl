...
function filter{S<:AbstractFloat}(m::AbstractModel, df::DataFrame, system::System{S},
    ...
    cond_type::Symbol = :none, allout::Bool = false, include_presample::Bool = true)

    data = df_to_matrix(m, df; cond_type = cond_type)
    filter(m, data, system, z0, P0; allout = allout, include_presample = include_presample)
end
...
function filter{S<:AbstractFloat}(m::AbstractModel, data::Matrix{S},
    ...
    if isnull(system)
        ...
        filter(m, data, get(system), z0, P0;
