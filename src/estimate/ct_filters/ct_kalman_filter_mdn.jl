...
function ct_kalman_filter(y::Matrix{S},
    ...
    k = Kalman(T, R, C, Q, Z, D, E, s_0, P_0)
...
Forecast functions
"""
function forecast!(k::Kalman{S}, tspan::Float64; method = Tsit5(),
                   reltol::Float64 = 1e-8, abstol::Float64 = 1e-8) where {S<:AbstractFloat}
    T, R, Q = k.T, k.R, k.Q
    s0, P0 = k.s_t, k.P_t
    f(u,p,t) = T*u
    ...
    s_sol = solve(s_prob, method, reltol = reltol, abstol = abstol)
    P_sol = solve(P_prob, method, reltol = reltol, abstol = abstol)
    k.s_t = s_sol.u
    k.P_t = P_sol.u
end
...
function forecast!(k::Kalman{S}, tspan::Float64, n_subinterval::Int64; method = Tsit5(),
                   reltol::Float64 = 1e-8, abstol::Float64 = 1e-8) where {S<:AbstractFloat}
    T, R, Q = k.T, k.R, k.Q
