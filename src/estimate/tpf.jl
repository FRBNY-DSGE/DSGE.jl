...
#        @show lik
 ...
 #       @show perror, check_InEff
        ...
        while (check_InEff>rstar)
            ...
            if prod(sign(fphi_interval))==-1
               ...
               # c = updateC(c,acpt_rate,trgt)
                c *= (0.95 + 0.10*exp( 16*(acpt_rate-trgt) ) / (1 + exp( 16*(acpt_rate-trgt) )) )
                @show c
                μ = mean(ε_up,2)
                ...
                @show s_fore, ε_up, acptVec
                # Calculate average accept rate
                acpt_rate = mean(acptVec)
                println("Updated acpt_rate after taking mean")
                @show acpt_rate
       ...
       # c = updateC(c,acpt_rate,trgt)
        c *= (0.95+0.1*exp(16*(acpt_rate-trgt))/(1+exp(16*(acpt_rate-trgt))))
        @show acpt_rate-trgt
        μ = mean(ε_up,2)
        cov_s = (1/numParticles)*(ε_up-repmat(μ,1,numParticles))*(ε_up - repmat(μ,1,numParticles))'
#=
...
function updateC(c_in::Float64, acpt_in::Float64, trgt_in::Float64)
    return c_in*(0.95+0.1*exp(16*(acpt_in-trgt_in))/(1+exp(16*(acpt_in-trgt_in))))
end
