...
function smc(m::AbstractModel, data::Matrix; verbose::Symbol=:low)
    ...
    tune = Tune(length(m.parameters),n_part,N_phi,2.,[],6,0.5,0.25,0.25,[], [], [], [])
    ...
    open("draws.csv","a") do x
        writecsv(x,reshape(parasim[i,:,:],(n_part,n_params)))
    end 
...
if i % 1 == 0 
    ...
    open("loglh.csv","a") do x
        writecsv(x,reshape(loglh,(1,tune.npart)))
        writecsv(x,reshape(logpost,(1,tune.npart)))
        write(x,"\n")
    end
