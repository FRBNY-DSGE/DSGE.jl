...
function estimate(m::AbstractVARModel, df::DataFrame;
                  verbose::Symbol = :low,
                  proposal_covariance::Matrix = Matrix(undef, 0, 0),
                  mle::Bool = false,
                  sampling::Bool = true,
                  ...
                  intermediate_stage_start::Int = 0,
                  intermediate_stage_increment::Int = 10,
                  save_intermediate::Bool = false, λ::Float64 = 0.)
    data = df_to_matrix(m.dsge, df)
    estimate(m, data; verbose = verbose, proposal_covariance = proposal_covariance,
             mle = mle, sampling = sampling,
             intermediate_stage_increment = intermediate_stage_increment,
             save_intermediate = save_intermediate)
...
function estimate(m::AbstractVARModel, data::AbstractArray;
                  verbose::Symbol = :low,
                  proposal_covariance::Matrix = Matrix(undef, 0,0),
                  mle::Bool = false,
                  sampling::Bool = true,
    ...
    if isa(m, DSGEVAR)
        if m.λ == 0
            error("Estimating a DSGEVAR with λ = 0 has not been implemented.")
        end
    end
    ...
    if reoptimize(_m) && method == :MH
        ...
        while !converged
            ...
            # Write params to file after every `n_iterations` iterations
            params = map(θ->θ.value, _m.parameters)
            h5open(rawpath(m, "estimate", "paramsmode.h5"),"w") do file
                file["params"] = params
            end
        end
    ...
    if get_setting(m, :sampling_method) == :MH
        ...
        ## Calculate the Hessian at the posterior mode
        hessian = if calculate_hessian(_m)
            println(verbose, :low, "Recalculating Hessian...")
        ...
        if rank(propdist) != n_parameters_free(_m)
            println("problem –    shutting down dimensions")
        end
