...
function default_settings!(m::AbstractModel)
   ...
   settings[:optimization_ftol] = Setting(:optimization_ftol, 1e-10, "The relative function difference threshold for optimization")
    ...
    settings[:c] = Setting(:c, .5, "The scaling factor for the covariance, constructed from accept and tgt during mutation")
    ...
    settings[:tpf_target] = Setting(:tpf_target, 0.25, "Initial target acceptance rate for new particles during mutation")
