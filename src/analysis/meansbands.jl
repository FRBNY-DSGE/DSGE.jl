...
# Define arithmetic operators for MeansBands objects
for op in (:(Base.:+),
           :(Base.:-))
    @eval begin
        function $op(mb1::MeansBands, mb2::MeansBands)
            mb = deepcopy(mb1)
            colnames = collect(keys(mb.bands))
            try
                for col in colnames
                    mb.means[!,col] = $op(mb.means[!,col], mb2.means[!,col])
                end
            catch means_err
                println("DataFrame of means for `mb1` and `mb2` must match.")
                println("Rethrowing original error for details...")
                rethrow(means_err)
            end
            try
                band_pcts = setdiff(names(mb.bands[colnames[1]]), [:date])
                for col in colnames
                    for band_pct in band_pcts
                        mb.bands[col][!,band_pct] = $op(mb.bands[col][!,band_pct],
                                                        mb2.bands[col][!,band_pct])
                    end
                end
            catch bands_err
                println("Dictionary of bands for `mb1` and `mb2` must match.")
                println("Rethrowing original error for details...")
                rethrow(bands_err)
            end
...
# Define multiplication, division, and exponentiation
# under assumption you want to do element-wise operations
for op in (:(Base.:*),
           :(Base.:/),
           :(Base.:^))
    @eval begin
        function $op(mb1::MeansBands, mb2::MeansBands)
            ...
            try
                for col in colnames
                    mb.means[!,col] = map($op, mb.means[!,col], mb2.means[!,col])
                end
            catch means_err
                println("DataFrame of means for `mb1` and `mb2` must match.")
                println("Rethrowing original error for details...")
            ...
            try
                ...
                for col in colnames
                    for band_pct in band_pcts
                        mb.bands[col][!,band_pct] = map($op, mb.bands[col][!,band_pct],
                                                        mb2.bands[col][!,band_pct])
                    end
                end
            catch bands_err
