...
function decomposition_means(m_new::AbstractModel, m_old::AbstractModel,
    ...
    # Read in dates
    input_file = get_decomp_output_files(m_new, m_old, input_type, cond_new, cond_old,
                                         [class])[Symbol(:decomptotal, class)]
...
function get_decomp_mean_file(m_new::AbstractModel, m_old::AbstractModel,
                              input_type::Symbol, cond_new::Symbol, cond_old::Symbol,
                              class::Symbol)
    fns = get_decomp_output_files(m_new, m_old, input_type, cond_new, cond_old, [class])
    fn = fns[Symbol(:decomp, :total, class)]
    fn = replace(fn, "total", "")
    fn = replace(fn, "raw", "work")
end
