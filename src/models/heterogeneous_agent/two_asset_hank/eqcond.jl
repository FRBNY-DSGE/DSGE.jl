...
function eqcond(m::TwoAssetHANK)
    ...
    @inline function get_residuals(vars::Vector{T}) where {T<:Real}
        ...
        audriftB, budriftB, audriftF, budriftB, adriftB, bdriftB, adriftF, bdriftF = catch_my_drifts(permanent, death, pam, xxi, d_g, a_g_grid, r_a_g_grid, w, l_g_grid, y_g_grid, d_g, a_g_grid, s_g, chi0, chi1, chi2, a_lb, a_grid, r_a_grid, l_grid, y_grid, aggZ, d, s)

        # Derive transition matrices
        aa, bb, aau, bbu = transition_deriva(I_g, J_g, N, I, J, ddeath, pam, xxi, w, chi0, chi1,
        ...
        elseif distributional_variables == 1
                C_Var_out = sum(log(c_g[:]).^2 .* gg .* dab_g[:]) - sum(log(c_g[:]) .*
                                                                        gg .* dab_g[:])^2
