...
originally written in MATLAB by Ben Moll and SeHyoun Ahn."
...
function init_parameters!(m::TwoAssetHANK)
   ...
   m <= SteadyStateParameterArray(:NHTM_indicator, Vector{Float64}(),
    ...
    m <= SteadyStateParameterArray(:var_SS, Vector{Float64}(),
                                   description = "")
end
...
#<<<<<<
    ...
    for ii	= 1 : maxit_KL
        ...
        for nn    = 1 : maxit_HJB
            ...
            for kk = 1:N
            	...
            	Bk 				= (1 + Delta*(rrho + ddeath) - Delta*lambda[kk,kk])*my_speye(I*J)
                                   - Delta*Ak
    ...
    m[:IcBB_SS]	 = IcBB
    m[:Ic00_SS]	 = Ic00
    m <= Setting(:a_g_0pos, a_g_0pos[1])
    m <= Setting(:b_g_0pos, b_g_0pos[1])
