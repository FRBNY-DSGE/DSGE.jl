...
function measurement(m::Model994, TTT::Matrix, RRR::Matrix, CCC::Matrix; shocks::Bool = true)
    ...
    ZZ[obs[:g_y], endo[:e_gdp_t]]   = 1.0
    ZZ[obs[:g_y], endo[:e_gdp_t1]]  = -me_level
    DD[obs[:g_y]]                   = 100*(exp(m[:zstar])-1)

    # Loading on GDY
    ZZ[obs[:g_income],endo[:y_t]]      =  m[:γ_gdy]
    ZZ[obs[:g_income],endo[:y_t1]]     = -m[:γ_gdy]
    ZZ[obs[:g_income],endo[:z_t]]      =  m[:γ_gdy]
    ZZ[obs[:g_income],endo[:e_gdy_t]]  = 1.0
    ZZ[obs[:g_income],endo[:e_gdy_t1]] = -m[:me_level]
    DD[obs[:g_income]] = 100*(exp(m[:zstar])-1) + m[:δ_gdy]
    ...
    QQ[exo[:gdy_sh], exo[:gdy_sh]]       = m[:σ_gdy]^2
