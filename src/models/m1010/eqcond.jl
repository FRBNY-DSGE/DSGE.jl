...
function eqcond(m::Model1010)
    ...
    Γ1[eq[:eq_nevol], endo[:b_liq_t]]  = m[:ζ_nR]*((m[:σ_c]*(1.0+m[:h]*exp(-m[:z_star])))/(1.0-m[:h]*exp(-m[:z_star])))
    Γ1[eq[:eq_nevol], endo[:b_safe_t]] = m[:ζ_nR]*((m[:σ_c]*(1.0+m[:h]*exp(-m[:z_star])))/(1.0-m[:h]*exp(-m[:z_star])))
    ...
    Γ1[eq[:eq_nevol_f], endo[:b_liq_t]]  = m[:ζ_nR]*((m[:σ_c]*(1.0+m[:h]*exp(-m[:z_star])))/(1.0-m[:h]*exp(-m[:z_star])))
    Γ1[eq[:eq_nevol_f], endo[:b_safe_t]] = m[:ζ_nR]*((m[:σ_c]*(1.0+m[:h]*exp(-m[:z_star])))/(1.0-m[:h]*exp(-m[:z_star])))
