...
The `Model1010` type defines the structure of Model1010 (same as
...
function init_model_indices!(m::Model1010)
    ...
    endogenous_states = [[
        ...
        :π_t, :μ_ω_t, :w_t, :L_t, :R_t, :g_t, :b_liq_t, :b_safe_t, :μ_t, :z_t, :λ_f_t, :λ_f_t1,
    ...
    equilibrium_conditions = [[
        ...
        :eq_mkupp, :eq_phlps, :eq_caprnt, :eq_msub, :eq_wage, :eq_mp, :eq_res, :eq_g, :eq_b_liq, :eq_b_safe,
        :eq_μ, :eq_z, :eq_λ_f, :eq_λ_w, :eq_rm, :eq_σ_ω, :eq_μ_e, :eq_γ, :eq_λ_f1, :eq_λ_w1, :eq_Ec,
...
function Model1010(subspec::AbstractString="ss18";
    ...
    m <= parameter(:lnb_safe, 0.26/4,  (1e-5, 10.),   (1e-5, 10.), DSGE.Exponential(), GammaAlt(0.26/4, 0.05),
                   fixed=true, scaling = x -> (1 + x/100),
    ...
    m <= parameter(:ρ_g,      0.9863, (0.0, 1.0),   (0.0, 1.0), DSGE.SquareRoot(),    BetaAlt(0.5, 0.2),
                   fixed=false,
    ...
    m <= parameter(:ρ_z,      0.9446, (0.0, 1.0),   (0.0, 1.0), DSGE.SquareRoot(),    BetaAlt(0.5, 0.2),
                   fixed=false,
                   description="ρ_z: AR(1) coefficient in the technology process.",
                   tex_label="\\rho_z")

    m <= parameter(:ρ_λ_f,    0.8827, (0.0, 1.0),   (0.0, 1.0), DSGE.SquareRoot(),    BetaAlt(0.5, 0.2),
                   fixed=false,
    ...
    m <= parameter(:ρ_σ_w,   0.9898, (0.0, 1.0), (0.0, 1.0),  DSGE.SquareRoot(),    BetaAlt(0.75, 0.15),
                   fixed=false,
                   description="ρ_σ_w: The standard deviation of entrepreneurs' capital productivity follows
                an exogenous process with mean ρ_σ_w. Innovations to the process are called _spread shocks_.",
                   tex_label="\\rho_{\\sigma_\\omega}")

    m <= parameter(:ρ_μ_e,    0.7500, (0.0, 1.0), (0.0, 1.0),  DSGE.SquareRoot(),    BetaAlt(0.75, 0.15),
                   fixed=true,
                   description="ρ_μ_e: AR(1) coefficient in the exogenous bankruptcy cost process.",
                   tex_label="\\rho_{\\mu_e}")

    m <= parameter(:ρ_γ,      0.7500, (0.0, 1.0), (0.0, 1.0),  DSGE.SquareRoot(),    BetaAlt(0.75, 0.15),
                   fixed=true,
                   description="ρ_γ: AR(1) coefficient in the process describing the fraction of entrepreneurs
                    surviving period t.",
                   tex_label="\\rho_{\\gamma}")

    m <= parameter(:ρ_π_star,  0.9900, (0.0, 1.0),      (0.0, 1.0), DSGE.SquareRoot(), BetaAlt(0.5, 0.2),
                   fixed=true,
                   description="ρ_π_star: No description available.",
                   tex_label="\\rho_{\\pi^*}")
...
m <= parameter(:ρ_tfp,     0.1953, (0.0, 1.0),      (0.0, 1.0), DSGE.SquareRoot(),    BetaAlt(0.5, 0.2),           fixed=false, description="ρ_tfp: No description available.",            tex_label="\\rho_{tfp}")
    ...
    m <= parameter(:η_gz,       0.8400, (0.0, 1.0), (0.0, 1.0), DSGE.SquareRoot(),
                   BetaAlt(0.50, 0.20), fixed=false,
                   description="η_gz: Correlate g and z shocks.",
                   tex_label="\\eta_{gz}")

    m <= parameter(:η_λ_f,      0.7892, (0.0, 1.0), (0.0, 1.0), DSGE.SquareRoot(),
                   BetaAlt(0.50, 0.20),         fixed=false,
                   description="η_λ_f: No description available.",
                   tex_label="\\eta_{\\lambda_f}")

    m <= parameter(:η_λ_w,      0.4226, (0.0, 1.0), (0.0, 1.0), DSGE.SquareRoot(),
                   BetaAlt(0.50, 0.20),         fixed=false,
