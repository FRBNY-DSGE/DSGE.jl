...
function eqcond(m::SmetsWoutersOrig)
    ...
    Γ0[eq[:eq_euler], endo[:Ec_t]]   = -1/(1 + m[:h]/m[:γ])
    Γ0[eq[:eq_euler], endo[:L_t]]    = -(m[:σ_c] - 1)*m[:wl_c]/(m[:σ_c]*(1 + m[:h]/m[:γ]))
    Γ0[eq[:eq_euler], endo[:EL_t]]   = (m[:σ_c] - 1)*m[:wl_c]/(m[:σ_c]*(1 + m[:h]/m[:γ]))
    Γ0[eq[:eq_euler], endo[:R_t]]    = (1 - m[:h]/m[:γ])/(m[:σ_c]*(1 + m[:h]/m[:γ]))
    Γ0[eq[:eq_euler], endo[:Eπ_t]]   = -(1 - m[:h]/m[:γ])/(m[:σ_c]*(1 + m[:h]/m[:γ]))
    Γ0[eq[:eq_euler], endo[:b_t]]    = -1.
    Γ1[eq[:eq_euler], endo[:c_t]]    = (m[:h]/m[:γ])/(1 + m[:h]/m[:γ])

    # Flexible prices and wages
    Γ0[eq[:eq_euler_f], endo[:c_f_t]]  = 1.
    Γ0[eq[:eq_euler_f], endo[:Ec_f_t]] = -1/(1 + m[:h]/m[:γ])
    Γ0[eq[:eq_euler_f], endo[:L_f_t]]  = -(m[:σ_c] - 1)*m[:wl_c]/(m[:σ_c]*(1 + m[:h]/m[:γ]))
    Γ0[eq[:eq_euler_f], endo[:EL_f_t]] = (m[:σ_c] - 1)*m[:wl_c]/(m[:σ_c]*(1 + m[:h]/m[:γ]))
    Γ0[eq[:eq_euler_f], endo[:r_f_t]]  = (1 - m[:h]/m[:γ])/(m[:σ_c]*(1 + m[:h]/m[:γ]))
    Γ0[eq[:eq_euler_f], endo[:b_t]]    = -1.
    Γ1[eq[:eq_euler_f], endo[:c_f_t]]  = (m[:h]/m[:γ])/(1 + m[:h]/m[:γ])
    ...
    Γ0[eq[:eq_inv], endo[:Ei_t]] = -m[:β]*m[:γ]^(1 - m[:σ_c])/(1 + m[:β]*m[:γ]^(1 - m[:σ_c]))
    Γ0[eq[:eq_inv], endo[:qk_t]] = -1 / (m[:S′′] * m[:γ]^2 * (1 + m[:β]*m[:γ]^(1 - m[:σ_c])))
    Γ0[eq[:eq_inv], endo[:μ_t]]  = -1.
    Γ1[eq[:eq_inv], endo[:i_t]]  = 1/(1 + m[:β]*m[:γ]^(1 - m[:σ_c]))

    # Flexible prices and wages
    Γ0[eq[:eq_inv_f], endo[:i_f_t]]  = 1.
    Γ0[eq[:eq_inv_f], endo[:Ei_f_t]] = -m[:β]*m[:γ]^(1 - m[:σ_c])/(1 + m[:β]*m[:γ]^(1 - m[:σ_c]))
    Γ0[eq[:eq_inv_f], endo[:qk_f_t]] = -1 / (m[:S′′] * m[:γ]^2 * (1 + m[:β]*m[:γ]^(1 - m[:σ_c])))
    Γ0[eq[:eq_inv_f], endo[:μ_t]]    = -1.
    Γ1[eq[:eq_inv_f], endo[:i_f_t]]  = 1/(1 + m[:β]*m[:γ]^(1 - m[:σ_c]))
    ...
    Γ0[eq[:eq_capval],endo[:Eqk_t]] = -m[:β] * m[:γ]^(-m[:σ_c]) * (1 - m[:δ])
    Γ0[eq[:eq_capval],endo[:Erk_t]] = -(1 - m[:β] * m[:γ]^(-m[:σ_c]) * (1 - m[:δ]))
    Γ0[eq[:eq_capval],endo[:R_t]]   = 1.
    Γ0[eq[:eq_capval],endo[:Eπ_t]]  = -1.
    Γ0[eq[:eq_capval],endo[:b_t]]   = -1 / ((1-m[:h]/m[:γ]) / ((1+m[:h]/m[:γ])* m[:σ_c]))

    # Flexible prices and wages
    Γ0[eq[:eq_capval_f], endo[:qk_f_t]]  = 1.
    Γ0[eq[:eq_capval_f], endo[:Eqk_f_t]] = -m[:β] * m[:γ]^(-m[:σ_c]) * (1 - m[:δ])
    Γ0[eq[:eq_capval_f], endo[:Erk_f_t]] = -(1 - m[:β] * m[:γ]^(-m[:σ_c]) * (1 - m[:δ]))
    Γ0[eq[:eq_capval_f], endo[:r_f_t]]   = 1.
    Γ0[eq[:eq_capval_f], endo[:b_t]]     = -1 / ((1-m[:h]/m[:γ]) / ((1+m[:h]/m[:γ])* m[:σ_c]))
    ...
    Γ0[eq[:eq_capev], endo[:μ_t]]    = -(1 - (1 - m[:δ])/m[:γ]) * m[:γ]^2 * m[:S′′] * (1 + m[:β]*m[:γ]^(1 - m[:σ_c]))
    Γ0[eq[:eq_capev], endo[:i_t]]    = -(1 - (1 - m[:δ])/m[:γ])
    Γ1[eq[:eq_capev], endo[:kbar_t]] = (1 - m[:δ])/m[:γ]

    # Flexible prices and wages
    Γ0[eq[:eq_capev_f], endo[:kbar_f_t]] = 1.
    Γ0[eq[:eq_capev_f], endo[:μ_t]]      = -(1 - (1 - m[:δ])/m[:γ]) * m[:γ]^2 * m[:S′′] * (1 + m[:β]*m[:γ]^(1 - m[:σ_c]))
    Γ0[eq[:eq_capev_f], endo[:i_f_t]]    = -(1 - (1 - m[:δ])/m[:γ])
    Γ1[eq[:eq_capev_f], endo[:kbar_f_t]] = (1 - m[:δ])/m[:γ]
    ...
    Γ0[eq[:eq_msub], endo[:c_t]]   = 1/(1 - m[:h]/m[:γ])
    Γ1[eq[:eq_msub], endo[:c_t]]   = (m[:h]/m[:γ]) / (1 - m[:h]/m[:γ])

    # Flexible prices and wages
    Γ0[eq[:eq_msub_f], endo[:w_f_t]] = -1.
    Γ0[eq[:eq_msub_f], endo[:L_f_t]] = m[:ν_l]
    Γ0[eq[:eq_msub_f], endo[:c_f_t]] = 1/(1 - m[:h]/m[:γ])
    Γ1[eq[:eq_msub_f], endo[:c_f_t]] = (m[:h]/m[:γ]) / (1 - m[:h]/m[:γ])
    ...
    Γ0[eq[:eq_wage], endo[:Ew_t]]   = -(1 - 1/(1 + m[:β]*m[:γ]^(1 - m[:σ_c])))
    Γ0[eq[:eq_wage], endo[:Eπ_t]]   = -(1 - 1/(1 + m[:β]*m[:γ]^(1 - m[:σ_c])))
    Γ0[eq[:eq_wage], endo[:π_t]]    = (1 + m[:β]*m[:γ]^(1 - m[:σ_c])*m[:ι_w]) / (1 + m[:β]*m[:γ]^(1 - m[:σ_c]))
    Γ0[eq[:eq_wage], endo[:μ_ω_t]]  = ((1 - m[:β]*m[:γ]^(1 - m[:σ_c])*m[:ζ_w]) * (1 - m[:ζ_w])) /
        ((1 + m[:β]*m[:γ]^(1 - m[:σ_c])) * (m[:ζ_w]*((m[:λ_w] - 1)*m[:ϵ_w] + 1)))
    Γ0[eq[:eq_wage], endo[:λ_w_t]]  = -1.
    Γ1[eq[:eq_wage], endo[:w_t]]    = 1/(1 + m[:β]*m[:γ]^(1 - m[:σ_c]))
    Γ1[eq[:eq_wage], endo[:π_t]]    = m[:ι_w]/(1 + m[:β]*m[:γ]^(1 - m[:σ_c]))
    ...
    Γ0[eq[:eq_res], endo[:c_t]] = -m[:c_y]
    Γ0[eq[:eq_res], endo[:i_t]] = -m[:i_y]
    Γ0[eq[:eq_res], endo[:u_t]] = -m[:u_y]
    Γ0[eq[:eq_res], endo[:g_t]] = -1.

    # Flexible prices and wages
    Γ0[eq[:eq_res_f], endo[:y_f_t]] = 1.
    Γ0[eq[:eq_res_f], endo[:c_f_t]] = -m[:c_y]
    Γ0[eq[:eq_res_f], endo[:i_f_t]] = -m[:i_y]
    Γ0[eq[:eq_res_f], endo[:u_f_t]] = -m[:u_y]
    Γ0[eq[:eq_res_f], endo[:g_t]]   = -1.
