"""
```
pseudo_measurement(m::SmetsWouters{T},
...
```
"""
function pseudo_measurement(m::SmetsWouters{T},
                            TTT::Matrix{T},
                            RRR::Matrix{T},
    ...
    DD_pseudo[pseudo[:NominalWageGrowth]]            = 100. * (m[:π_star] - 1.) + 100. * (exp(m[:zstar]) - 1.)
    ...
    DD_pseudo[pseudo[:LaborProductivityGrowthNoME]]                   = 100. * (exp(m[:zstar]) - 1.)
