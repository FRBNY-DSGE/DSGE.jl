"""
```
ClaridaGaliGertler{T} <: AbstractRepModel{T}
```

The `ClaridaGaliGertler` type defines the structure of the simple New Keynesian DSGE
model described in Clarida, Gali, and Gertler (1999).
...
* `spec::String`: The model specification identifier, \"clarida_gali_gertler\", cached
...
"""
mutable struct ClaridaGaliGertler{T} <: AbstractRepModel{T}
...
description(m::ClaridaGaliGertler) = "Julia implementation of model defined in Clarida, Gali, Gertler (1999): ClaridaGaliGertler, $(m.subspec)"

"""
`init_model_indices!(m::ClaridaGaliGertler)`

Arguments:
`m:: ClaridaGaliGertler`: a model object

Description:
Initializes indices for all of `m`'s states, shocks, and equilibrium conditions.
"""
function init_model_indices!(m::ClaridaGaliGertler)
    # Endogenous states
    endogenous_states = collect([
        :y_t, :π_t, :R_t, :R_star_t, :g_t, :z_t, :Ey_t1, :Eπ_t1, :y_t1])
    ...
    equilibrium_conditions = collect([
        :eq_euler, :eq_phillips, :eq_mp, :eq_mprule, :eq_g, :eq_z, :eq_Ey, :eq_Eπ, :eq_y_t1])
...
function ClaridaGaliGertler(subspec::String="ss0";
...
"""
```
init_parameters!(m::ClaridaGaliGertler)
```
...
those).
"""
function init_parameters!(m::ClaridaGaliGertler)
    # Initialize parameters
...
"""
```
steadystate!(m::ClaridaGaliGertler)
```

Calculates the model's steady-state values. `steadystate!(m)` must be called whenever
the parameters of `m` are updated.
"""
function steadystate!(m::ClaridaGaliGertler)
    return m
end

function model_settings!(m::ClaridaGaliGertler)
    default_settings!(m)
...
function shock_groupings(m::ClaridaGaliGertler)
