...
"""
function eqcond(m::ClaridaGaliGertler)
    ...
    elseif subspec(m) == "ss1"
        Γ0[eq[:eq_phillips], endo[:Eπ_t1]] = -exp(m[:γ_Q] / 100) / exp(m[:rA] / 100)
    end

    ### 3. Monetary Policy Rule

    Γ0[eq[:eq_mp], endo[:R_star_t]] = -(1. - m[:ρ_R])
    ...
    Γ0[eq[:eq_mprule], endo[:y_t]] = -m[:ψ_2]
    Γ0[eq[:eq_mprule], endo[:π_t]] = -m[:ψ_1]
    Γ0[eq[:eq_mprule], endo[:R_star_t]] = 1
    if subspec(m) == "ss0"
        Γ0[eq[:eq_mprule], endo[:g_t]] = m[:ψ_2]
    elseif subspec(m) == "ss1"
        Γ0[eq[:eq_mprule], endo[:g_t]] = 0.
    end
