"""
```
measurement(m::ClaridaGaliGertler{T}, TTT::Matrix{T},
...
```
"""
function measurement(m::ClaridaGaliGertler{T},
                     TTT::Matrix{T},
                     RRR::Matrix{T},
    ...
    elseif subspec(m) == "ss1"
        DD[obs[:obs_nominalrate]]         = 4. * (m[:π_star] + m[:rA])
    end
