...
function filter{S<:AbstractFloat}(m::AbstractModel,
    ...
    function setup_filter(m,data,lead,z0,vz0,Ny0,allout,use_expected_rate_data)
        m=m
        data=data
        lead=lead
        z0=z0
    ...
    @everywhere setup_filter(m,data,lead,z0,vz0,Ny0,allout,use_expected_rate_data)
    inputs = @sync @parallel for i = 1:ndraws
    ...
    ## for i = 1:ndraws
    ##     filtered_states[i] = filter(m, data, lead, sys[i], z0, vz0, Ny0, allout, use_expected_rate_data) 
    ## end
        
    return filtered_states
end

type FilterInput{S<:AbstractFloat}
    m::AbstractModel
    data::Matrix{S}
    lead::Int
    sys::System
...
function filter{S<:AbstractFloat}(m::AbstractModel,
    ...
    if use_expected_rate_data
        ...
        filtered_states = append!(R2[:filt],R3[:filt])
...
function kalman_filter_2part(m::AbstractModel,
                             data::Matrix{S},
                             TTT=[],
                             RRR=[],
                             CCC=[],
