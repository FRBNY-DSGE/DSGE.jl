...
function smooth_ait_gdp_replace_eq_entries(m::AbstractDSGEModel,
...
function smooth_ait_gdp_eqcond(m::AbstractDSGEModel, reg::Int = 1)
    ...
    Γ0, Γ1, C, Ψ, Π = smooth_ait_gdp_replace_eq_entries(m, Γ0, Γ1, C, Ψ, Π)
...
"""
```
smooth_ait_gdp_solve(m::AbstractDSGEModel)
```

Solves for the transition equation of `m` under a price level
targeting rule (implemented by adding a price-gap state)
"""
function smooth_ait_gdp_solve(m::AbstractDSGEModel; regime_switching::Bool = false, regimes::Union{Int, Vector{Int}, UnitRange{Int}} = 1)
    # Get equilibrium condition matrices

    if isa(regimes, Int)
        Γ0, Γ1, C, Ψ, Π  = smooth_ait_gdp_eqcond(m, regimes)
...
"""
```
init_smooth_ait_gdp_forecast(m::AbstractDSGEModel, shocks::Matrix{T}, final_state::Vector{T})
```

Adjust shocks matrix and final state vector for forecasting under the SMOOTH_AIT_GDP rule
"""
function smooth_ait_gdp_forecast_init(m::AbstractDSGEModel, shocks::Matrix{T}, final_state::Vector{T};
