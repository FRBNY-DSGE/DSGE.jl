...
This rule uses an alternative specification of coefficients compared to `smooth_ait_gdp`.
"""
function smooth_ait_gdp_alt()
    AltPolicy(:smooth_ait_gdp_alt, smooth_ait_gdp_alt_eqcond, smooth_ait_gdp_alt_solve,
              replace_eqcond = smooth_ait_gdp_alt_replace_eq_entries,
              forecast_init = smooth_ait_gdp_alt_forecast_init,
              color = RGB(0., 0., 0.5430)) # dark blue
end

function smooth_ait_gdp_alt_replace_eq_entries(m::AbstractDSGEModel,
    ...
    ρ_smooth  = haskey(get_settings(m), :smooth_ait_gdp_alt_ρ_smooth) ? get_setting(m, :smooth_ait_gdp_alt_ρ_smooth) : 0.656 # m[:ρ]
    φ_π       = haskey(get_settings(m), :smooth_ait_gdp_alt_φ_π) ? get_setting(m, :smooth_ait_gdp_alt_φ_π) : 11.13
    φ_y       = haskey(get_settings(m), :smooth_ait_gdp_alt_φ_y) ? get_setting(m, :smooth_ait_gdp_alt_φ_y) : 11.13
...
"""
```
smooth_ait_gdp_alt_solve(m::AbstractDSGEModel)
```
...
function smooth_ait_gdp_alt_solve(m::AbstractDSGEModel; regime_switching::Bool = false, regimes::Vector{Int} = Int[1])
    return DSGE.altpolicy_solve(m, smooth_ait_gdp_alt(); regime_switching = regime_switching, regimes = regimes)
end

"""
```
init_smooth_ait_gdp_alt_forecast(m::AbstractDSGEModel, shocks::Matrix{T}, final_state::Vector{T})
```

Adjust shocks matrix and final state vector for forecasting under the SMOOTH_AIT_GDP_ALT rule
"""
function smooth_ait_gdp_alt_forecast_init(m::AbstractDSGEModel, shocks::Matrix{T}, final_state::Vector{T};
