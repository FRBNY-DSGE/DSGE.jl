...
function rw()
    AltPolicy(:rw, rw_eqcond, rw_solve,
              replace_eqcond = rw_replace_eq_entries,
              forecast_init = rw_forecast_init,
              color = RGB(0., 0., 0.5430)) # dark blue
end

function rw_replace_eq_entries(m::AbstractDSGEModel,
    ...
    Γ0[eq[:eq_mp], endo[:Rref_t]] = -1. # "standard" MP as refernece rate
    Γ0[eq[:eq_mp], endo[:rw_t]]   = -1. # This is the RW part
...
function rw_eqcond(m::AbstractDSGEModel, reg::Int = 1)
    ...
    Γ0, Γ1, C, Ψ, Π = rw_replace_eq_entries(m, Γ0, Γ1, C, Ψ, Π)
...
"""
```
rw_solve(m::AbstractDSGEModel)
```

Solves for the transition equation of `m` under a price level
targeting rule (implemented by adding a price-gap state)
"""
function rw_solve(m::AbstractDSGEModel; regime_switching::Bool = false, regimes::Vector{Int} = Int[1])
    return DSGE.altpolicy_solve(m, rw(); regime_switching = regime_switching, regimes = regimes)
end

"""
```
init_rw_forecast(m::AbstractDSGEModel, shocks::Matrix{T}, final_state::Vector{T})
```

Adjust shocks matrix and final state vector for forecasting under the RW rule
"""
function rw_forecast_init(m::AbstractDSGEModel, shocks::Matrix{T}, final_state::Vector{T};
