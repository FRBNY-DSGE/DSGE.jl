using DSGE, DSGEModels, StateSpaceRoutines, JLD, HetAgentDSGE
...
function steadystate!(m::Tauchen86,
                ehi::AbstractFloat,
                elo::AbstractFloat,
                XGRIDB::Vector,
                SGRIDB::Vector,
                ...
                Win::Vector{Float64},
                g::Vector{Float64},
                tol::Float64 = 1e-5, maxit::Int64 = 10)

    R::Float64   = m[:R]
    # don't know if the interest rate is a parameter or setting in this case
    βlo::Float64 = 0.4/R
    βhi::Float64 = 1/R
    β::Float64   = (βlo + βhi) / 2.0

    # turning "nx" to "nw," as Michael did earlier
    ...
    while abs(excess) > tol && count < maxit # clearing markets
        ...
        if abs(D[1]-1)>2e-1 # that's the tolerance we are allowing
            warn("your eigenvalue is ", D[1], " which is too far from 1, something is wrong")
        end
        μ = MSMXinv*real(V[:,1]) #Pick the eigen vecor associated with the largest
...
tic()
(ell, c, h, μ, β) = steadystate!(βlo, βhi, R, γ, sigs,
                            ehi, elo, XGRIDB, SGRIDB,
                            xwts, swts, Win, g)
toc()
#shutup
...
# need to fix derivative!!

# Make the Jacobian
JJ = zeros(4*nx+2,8*nx+4)
...
funops = 1:2 # which operators output a functionF1 = 1:nx*ns #euler eqn
F2 = nx*ns+1:2*nx*ns #KF
F3 = 2*nx*ns+1:2*nx*ns+1 #mkt ckr
F4 = 2*nx*ns+2:2*nx*ns+2 #z

#Create auxiliary variables
#c = min.(ell.^(-1.0/γ),xgrid) # Consumption Decision Rule
Wss = MPL(1.0,K)              # Steady state wages
Rss = MPK(1.0,K)              # Steady state Rk

#KF equation
KFmollificand = (repmat(xgrid,1,nx*ns) - Rss*repmat(xgrid'-c',nx,ns))/Wss - kron(sgrid',ones(nx,nx))
μRHS = mollifier.(KFmollificand,ehi,elo)*kron(swts.*(g/Wss),xwts.*μ)
    ...
    for is =1:ns
        ...
        for ixp=1:nx
            for isp = 1:ns
                ...
                ξ[i,ip] = qp(mf)*g[isp]
                Ξ[i,ip] = (c[ip]⁽-Î-1))*qfunction(mf)*g[isp]*swts[isp]*xwts[ixp]
                LEE[i,ip] = Ξ[i,ip]*(ell[ip]⁽-1- 1/γ))*((ell[ip]⁽-1/γ))<=χss[ip])
                sumΓ+= (c[ip]⁽-Î))*ξ[i,ip]*swts[isp]*xwts[ixp]
                sumWEE += Ξ[i,ip]*chipW[ip]*((ell[ip]⁽-1/γ))>χss[ip])
                sumREE += Ξ[i,ip]*chipR[ip]*((ell[ip]⁽-1/γ))>χss[ip])
                sumELLRHS += β*R*(c[ip]⁽-Î))*qfunction(mf)*g[isp]*swts[isp]*xwts[ixp]
            end
        end
        Γ[i] = sumΓ
        WEE[i] = sumWEE
...
LLEE = (β/γ)*R*R*Γ.*(1+ (γ/ν)*SGRIDB.*η./c).*(ell.⁽-1-1/γ) ).*((ell.⁽-1/γ)).<=χss)-1
